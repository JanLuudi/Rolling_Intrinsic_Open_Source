...
CONNECTION = "postgres://elli@127.0.0.1/intradaydb"
CONNECTION_ALCHEMY = "postgresql://elli@127.0.0.1/intradaydb"
...
def get_average_prices(side, execution_time_start, execution_time_end, end_date, min_trades=10):
    ...
    cursor.execute(f"""
        ...
        AND (product ='XBID_Hour_Power' or product = 'Intraday_Hour_Power') AND side='{side}' AND deliverystart < '{end_date}' AND deliverystart >= '{start_of_day}'
        GROUP BY 
        deliverystart
        HAVING 
        COUNT(*) >= {min_trades};
...
def get_prices_day(df, execution_time_start, day):
    ...
    # set index to be all 15 minute intervals from start_of_day to end_of_day, filling missing values with NaN
    filtered_df = filtered_df.reindex(pd.date_range(start_of_day, end_of_day, freq='60min'))
...
def run_optimization_quarterhours_repositioning(prices_qh, execution_time, cap, c_rate, roundtrip_eff, max_cycles, threshold, threshold_abs_min, discount_rate, prev_net_trades=pd.DataFrame(columns=["sum_buy", "sum_sell", "net_buy", "net_sell", 'product'])):
    ...
    adjusted_obj = [((current_sell_qh[i] * (prices_qh_adj.loc[i,'price'] - max(abs((threshold/100) * abs(prices_qh.loc[i,'price'])), threshold_abs_min)/2 - e)) - (current_buy_qh[i] * (prices_qh_adj_buy.loc[i,'price'] + max(abs((threshold/100) * abs(prices_qh.loc[i,'price'])), threshold_abs_min)/2 + e))) * 1.0/1.0 for i in prices_qh.index if not pd.isna(prices_qh.loc[i, 'price'])  and (prev_net_trades.loc[i,'net_buy'] < e and prev_net_trades.loc[i,'net_sell'] < e)]

    # Original objective component for cases where previous trades >= e
    original_obj = [(current_sell_qh[i] * (prices_qh.loc[i,'price'] - e) - current_buy_qh[i] * prices_qh.loc[i,'price']) * 1.0/1.0 for i in prices_qh.index if not pd.isna(prices_qh.loc[i, 'price']) and (prev_net_trades.loc[i,'net_buy'] >= e or prev_net_trades.loc[i,'net_sell'] >= e)]
    ...
    for i in prices_qh.index[1:]:
        m_battery += battery_soc[i] == battery_soc[previous_index] + net_buy[previous_index]*efficiency*1.0/1.0 - net_sell[previous_index]*1.0/1.0/efficiency, f"BatteryBalance_{i}"
        previous_index = i
    ...
    for i in prices_qh.index:
        ...
        if pd.isna(prices_qh.loc[i, 'price']):
            ...
            m_battery += net_sell[i]*1.0/efficiency/1.0 <= battery_soc[i], f"SellVsSOC_{i}"
    ...
    m_battery += lpSum(net_buy[i]*efficiency*1.0/1.0 for i in prices_qh.index) <= max_cycles * cap, "MaxCycles"
...
def get_net_trades(trades, end_date):
    ...
    net_trades = net_trades.reindex(pd.date_range(start_of_day, end_of_day, freq='60min'))
...
def simulate_period(start_day, end_day, threshold, threshold_abs_min, discount_rate, bucket_size, c_rate, roundtrip_eff, max_cycles):
    ...
    while(current_day < end_day):
        ...
        current_cycles += net_trades["net_buy"].sum() / 1.0 * roundtrip_eff ** 0.5
        ...
        profits_db = pd.DataFrame([[current_day, daily_profit, net_trades["net_buy"].sum() / 1.0 * roundtrip_eff ** 0.5]], columns=['day', 'profit', 'cycles'])
